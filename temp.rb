...
bot.message(start_with: '!rps') do |game|
  rps = RockPaperScissorsGame.new
  game.respond(rps.start_message)
  ...
  while bot_tries > 0 || player_tries > 0 # Start game
    ...
    if !choices.include?(p_choice)
      game.respond(rps.invalid_guess)
  ...
  if bot_tries == 0
    ...
    game.respond("Ahhh nice try. Maybe you will win next time.") #change to link
  end
end
